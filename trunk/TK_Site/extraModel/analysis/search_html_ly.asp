...
iimember=iimember+1
  ...
  <tr>
    ...
    <td width="640" height="25" align="left" bgcolor="#FFFFFF">&nbsp;&nbsp;<a href='search_lyhtml.asp?search_url=<%=rs("sjmp5_url_name")%>&search=<%=search%>'  target="_blank">
    <%=rs("sjmp5_url_name")%></a></td>
    <td width="250" align="left" bgcolor="#FFFFFF"><img src="images/smallbg.jpg" width=<%=fix(rs("sjmp5_url_hits")/key_tj*150)%> height=10>&nbsp;<%=fix(rs("sjmp5_url_hits")/key_tj*100)%>%&nbsp;&nbsp;(<font color="#FF0000"><%=rs("sjmp5_url_hits")%></font>次)&nbsp;</td>
  </tr><%
   j=j+1
  rs.movenext
  loop
