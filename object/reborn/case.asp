...
sql = "SELECT * FROM news where class1id = 4"
rs.OPEN sql,Conn,1,1
%>
<body onLoad="loadContent('<%=rs("id")%>');changeTitle('');">
<div id="container">
  ...
  <div id="content">
    <div id="contentLeft">
      ...
      <ul>
		...
		i = 1
		while not rs.eof%>
        <li><a href="javascript:loadContent('<%=rs("id")%>');changeTitle('<%=rs("title")%>');"><span class="fontGreen"><%if i < 10 then%>0<%end if%><%=i%>/</span><%=rs("title")%></a></li>
        <%
		i = i + 1
		rs.movenext
		wend
