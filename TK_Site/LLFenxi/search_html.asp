	...
	function date_init(obj,name)
{
  ...
  <tr>
    <td height="30" colspan="3" align="center" background="images/tab_bg.gif" bgcolor="#eeeeee"><b>受访页面排行榜</b>( <%=search%>有<font color="#FF0000">
<%
rs.close
...
iimember=iimember+1
  ...
  <tr>
    ...
    <td width="639" height="25" align="left" bgcolor="#FFFFFF">&nbsp;&nbsp;<a href="<%=rs("sjmp5_html_url")%>" title='<%=rs("sjmp5_html_url")%>'  target="_blank">
    <%=rs("sjmp5_html_url")%></a></td>
    <td width="251" align="left" bgcolor="#FFFFFF"><img src="images/smallbg.jpg" width=<%=fix(rs("sjmp5_html_hits")/key_tj*150)%> height=10>&nbsp;<%=fix(rs("sjmp5_html_hits")/key_tj*100)%>%&nbsp;&nbsp;(<font color="#FF0000"><%=rs("sjmp5_html_hits")%></font>次)&nbsp;</td>
  </tr><%
   j=j+1
  rs.movenext
  loop
