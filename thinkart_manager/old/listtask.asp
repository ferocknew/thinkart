...
sql = "SELECT * FROM tm_task where pj_finish <> 1"
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="220" valign="top" class="opArea"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td height="50" class="helpTitle bottomDotted1px">非项目任务</td>
      </tr>
      <tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="50%" align="left" class="bottomBorder1px"><table width="120" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td width="120" height="35" class="tabCard">+ 非项目任务列表</td>
                  </tr>
                </table></td>
                <td height="25" align="right" class="bottomBorder1px"><input name="create_pj5" type="button" class="setOptButtom" id="create_pj" value="新建" onclick="javascript:document.location='addtask.asp';" /></td>
              </tr>
            </table>
            <br />
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <%while not rs.eof%>
              <tr>
                <td align="center" class="inputTable"><input name="create_pj4" type="button" class="setOptButtom" id="create_pj4" value="删除" onclick="javascript:if(confirm('确认删除该项目？'))location.href='listtask.asp?del_id=<%=rs("id")%>'" /><input name="create_pj" type="button" class="setOptButtom" id="create_pj4" value="编辑" onclick="javascript:document.location='edittask.asp?edit_id=<%=rs("id")%>';" /></td>
                ...
                <%if rs("pj_apply") = 0 then%>
                	...
                	<input name="set_apply" type="button" class="setOptButtomLong" id="create_pj3" value="标记为接受" onclick="javascript:if(confirm('确认接受？'))location.href='listtask.asp?apply_id=<%=rs("id")%>'" /></td>
                	<%end if%>
				<%else%>
                已接受
                <%end if%>
                ...
                <%if rs("pj_finish") = 0 then%>
                	<%if session("tname") = rs("pj_people") then%>
                	<input name="set_finish" type="button" class="setOptButtomLong" id="create_pj2" value="标记为完成" onclick="javascript:if(confirm('确认完成？'))location.href='listtask.asp?finish_id=<%=rs("id")%>'" /></td>
					<%end if%>
              	<%else%>
                已完成
                <%end if%>
